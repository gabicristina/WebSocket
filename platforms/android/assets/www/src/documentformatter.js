 ...
 * 
 * @param {Vex.Flow.Document}
 *            Document object to retrieve information from
 * @constructor
 */
 ...
 * Params: m (measure #), s (stave #) Returns: x (number)
 */
...
Vex.Flow.DocumentFormatter.prototype.getStaveY = function(m, s) {
	// Default behavour: calculate from stave above this one (or 0 for top
	// stave)
	...
	if (s == 0) {
		...
		this.measuresInBlock.forEach(function(measures) {
			if (measures.indexOf(m) > -1) {
				var maxExtraSpace = 50 - (new Vex.Flow.Stave(0, 0, 500)
						.getYForLine(0));
				measures.forEach(function(measure) {
					var extra = this.getMinMeasureHeight(measure)[0];
					if (extra > maxExtraSpace)
						maxExtraSpace = extra;
				}, this);
 ...
 * Params: m (measure #), s (stave #) Returns: width (number) which should be
 ...
 * 
 * @param {Vex.Flow.Measure.Stave}
 *            Original stave object
 * @param {Number}
 *            x position
 * @param {Number}
 *            y position
...
Vex.Flow.DocumentFormatter.prototype.createVexflowStave = function(s, x, y, w) {
	...
	s.modifiers.forEach(function(mod) {
		...
		case "time":
			var time_sig;
			if (typeof mod.time == "string")
				time_sig = mod.time;
			else
				time_sig = mod.num_beats.toString() + "/"
 ...
 * document and store it in vfStaves.
 * 
 * @param {Number}
 *            Measure number
 * @param {Number}
...
Vex.Flow.DocumentFormatter.prototype.getStave = function(m, s) {
	...
	if (!stave)
		return undefined;
	var vfStave = this.createVexflowStave(stave, this.getStaveX(m, s), this
			.getStaveY(m, s), this.getStaveWidth(m, s));
	if (!(m in this.vfStaves))
		this.vfStaves[m] = [];
 ...
 * the proper Vex.Flow.Stave in staves (spanning multiple staves in a single
 * voice not currently supported.)
 * 
 * @param {Vex.Flow.Measure.Voice}
 *            Voice object
 * @param {Array}
 *            Vex.Flow.Staves to add the notes to
...
Vex.Flow.DocumentFormatter.prototype.getVexflowVoice = function(voice, staves) {
	...
	for (var i = 0; i < voice.notes.length; i++) {
		...
		var vfNote = this.getVexflowNote(voice.notes[i], {
			clef : clef
		});
		if (note.beam == "begin")
			beamedNotes = [ vfNote ];
		...
		if (note.tuplet) {
			...
			if (tupletNotes.length == tupletOpts.num_notes) {
				vexflowObjects
						.push(new Vex.Flow.Tuplet(tupletNotes, tupletOpts));
 ...
 * 
 * @param {Vex.Flow.Measure.Note}
 *            Note object
 * @param {Object}
 *            Options (currently only clef)
...
Vex.Flow.DocumentFormatter.prototype.getVexflowNote = function(note, options) {
	...
	if (note.stem_direction)
		note_struct.stem_direction = note.stem_direction;
...
Vex.Flow.DocumentFormatter.prototype.getMinMeasureWidth = function(m) {
	if (!(m in this.minMeasureWidths)) {
		...
		var vfStaves = measure.getStaves().map(
				function(stave) {
					var vfStave = this.createVexflowStave(stave, 0, 0, 500);
					var extraWidth = 500 - (vfStave.getNoteEndX() - vfStave
							.getNoteStartX());
		...
		var context = Vex.Flow.Renderer.bolsterCanvasContext(canvas
				.getContext("2d"));

		var allVfVoices = [];
		var startStave = 0; // stave for part to start on
		measure.getParts().forEach(
				function(part) {
					var numStaves = part.getNumberOfStaves();
					var partStaves = vfStaves.slice(startStave, startStave
							+ numStaves);
					part.getVoices().forEach(
							function(voice) {
								var vfVoice = this.getVexflowVoice(voice,
										partStaves)[0];
		...
		allVfVoices.forEach(function(v) {
			...
			if (numTickables > maxTickables)
				maxTickables = numTickables;
		});
		this.minMeasureWidths[m] = Vex.Max(50, maxExtraWidth + noteWidth
				+ maxTickables * 10 + 10);
		...
		var minHeights = [];
		// Initialize to zero
		for (var i = 0; i < vfStaves.length + 1; i++)
			minHeights.push(0);

		var i = -1; // allVfVoices consecutive by stave, increment for each new
					// stave
		var lastStave = null;
		...
		allVfVoices.forEach(function(v) {
			if (v.stave !== lastStave) {
				...
				lastBoundingBox = new Vex.Flow.BoundingBox(0, staveY, 500,
						staveH);
				lastStave = v.stave;
				i++;
			}
			...
			.forEach(
					function(connector) {
						if (!((options.system_start && connector.system_start)
								|| (options.system_end && connector.system_end) || connector.measure_start))
							return;
						var firstPart = connector.parts[0], lastPart = connector.parts[connector.parts.length - 1];
						var firstStave, lastStave;
						// Go through each part in measure to find the stave
						// index
						var staveNum = 0, partNum = 0;
						parts.forEach(function(part) {
							if (partNum == firstPart)
								firstStave = staveNum;
							if (partNum == lastPart)
								lastStave = staveNum + part.getNumberOfStaves()
										- 1;
							staveNum += part.getNumberOfStaves();
							partNum++;
						});
						if (isNaN(firstStave) || isNaN(lastStave))
							return;
						...
						if ((options.system_start && connector.system_start)
							...
							(new Vex.Flow.StaveConnector(vfStaves[firstStave],
									vfStaves[lastStave])).setType(type)
									.setContext(context).draw();
						}
						if (options.system_end && connector.system_end) {
							var stave1 = vfStaves[firstStave], stave2 = vfStaves[lastStave];
							var dummy1 = new Vex.Flow.Stave(stave1.x
									+ stave1.width, stave1.y, 100);
							var dummy2 = new Vex.Flow.Stave(stave2.x
									+ stave2.width, stave2.y, 100);
 ...
 * update as necessary if the width of the element changes, etc.
 * 
 * @param {Node}
 *            HTML node to draw inside
 * @param {Object}
 *            Subclass-specific options
 */

/**
 * Vex.Flow.DocumentFormatter.Liquid - default liquid formatter Fit measures
 * onto lines with a given width, in blocks of 1 line of music
 * 
 * @constructor
 */
Vex.Flow.DocumentFormatter.Liquid = function(document) {
	if (arguments.length > 0)
		Vex.Flow.DocumentFormatter.call(this, document);
...
Vex.Flow.DocumentFormatter.Liquid.prototype.getBlock = function(b) {
	...
	if (!this.measureX)
		this.measureX = new Array();
	if (!this.measureWidth)
		this.measureWidth = new Array();
	...
	if (this.getMinMeasureWidth(startMeasure) + start_x + 10 >= this.width) {
		...
		while (width < this.width && curMeasure < numMeasures) {
			...
			if (curMeasure != startMeasure)
				this.document.getMeasure(curMeasure).getStaves().forEach(
						function(s) {
		...
		var extraWidth = Math.floor(remainingWidth
